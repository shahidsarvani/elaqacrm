...
$(function() {
	...
	function operate_deletions(urls,paras1,ret_div){ 
				...
				if(isConfirm) {
					$.ajax({
						...
						success: function (response) {
							...
							swal({
								title: "Deleted!",
								text: "Your selected record(s) has been deleted!",
								confirmButtonColor: "#66BB6A",
								type: "success"
							});
							
							document.getElementById(ret_div).innerHTML = response;
							
							reload_datatable_button_init_custom();
					...
					swal({
						title: "Cancelled",
						text: "Your imaginary record safe!",
						confirmButtonColor: "#2196F3",
						type: "error"
					}); 
				} 
	 ...
	 function operate_multi_deletions(cstm_form_name){ 
		 $(document).ready(function() {    
			swal({
				title: "Are you sure?",
				text: "You will not be able to view this record!",
			...
			function(isConfirm){
				if (isConfirm) {  
					...
					var cstm_form_nam1 = '#'+cstm_form_name; 
					var form11 = $(cstm_form_nam1);
					// Serialize the form data.
					var formData = $(form11).serialize(); 
					// Submit the form using AJAX. 
					if(formData != "DataTables_Table_0_length=10"){   
						$.ajax({
							type: 'POST',
							...
							success: function (response) {
								...
								document.getElementById('dyns_list').innerHTML = response;
